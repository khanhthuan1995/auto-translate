...
def validate_file(ctx, param, value):
    if os.path.isfile(value):
        if mimetypes.guess_type(value)[0] in MIME :
            # click.echo(click.style(f"File Type accepted", bold=True, fg="green"))
            # print("hello")
            return value
        else: 
...
def handle_request(count:int, *args, **kwargs):
    try:
        ...
        if count == 0:
            print("Can not connect!!!! Retry after 20s")
            time.sleep(20)
            handle_request(5, *args, **kwargs)

        handle_request(count-1, *args, **kwargs)
...
def translate(data):
    ...
    response = handle_request(5, api_url, json=params)
    dataJson = response.json()
    print(dataJson['result']['script'])
...
@click.option('--input', prompt='Nhập tên tệp cần dịch', help='Tên tệp có chứa cả .docx nhé', callback=validate_file)
# @click.option('--output', prompt='Nhập tên sẽ được lưu',
#               help='Tên mới cần lưu',)
def docprocessing(input):
    ...
    with click.progressbar(document.tables) as bar:
        for t in bar:
            for r in t.rows:
                for c in r.cells:
                    for p in c.paragraphs:
                        transParagraphs(p)
    filename = "translated"+ input
